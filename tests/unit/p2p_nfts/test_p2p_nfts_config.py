...
def test_initial_state(
    ...
    assert p2p_nfts_eth.max_protocol_fee() == max_protocol_fee
    ...
    assert p2p_nfts_usdc.max_protocol_fee() == max_protocol_fee
...
def test_set_protocol_fee_reverts_if_not_owner(p2p_nfts_eth):
    with boa.reverts("not owner"):
        p2p_nfts_eth.set_protocol_fee(1, sender=boa.env.generate_address("random"))
...
def test_set_protocol_fee_reverts_if_protocol_fee_gt_max(p2p_nfts_eth, owner, max_protocol_fee):
    with boa.reverts("protocol fee > max fee"):
        p2p_nfts_eth.set_protocol_fee(max_protocol_fee + 1, sender=owner)


def test_set_protocol_fee(p2p_nfts_eth, owner, max_protocol_fee):
    p2p_nfts_eth.set_protocol_fee(max_protocol_fee, sender=owner)
    assert p2p_nfts_eth.protocol_fee() == max_protocol_fee

    p2p_nfts_eth.set_protocol_fee(0, sender=owner)
    assert p2p_nfts_eth.protocol_fee() == 0


def test_set_protocol_fee_logs_event(p2p_nfts_eth, owner):
    old_fee = p2p_nfts_eth.protocol_fee()
    new_fee = old_fee + 1

    p2p_nfts_eth.set_protocol_fee(new_fee, sender=owner)
