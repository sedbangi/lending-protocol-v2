...
@pytest.fixture
def ongoing_loan_bayc(p2p_nfts_eth, offer_bayc, weth, borrower, lender, bayc, now, borrower_broker, borrower_broker_fee, protocol_fee):
...
@pytest.fixture
def ongoing_loan_prorata(p2p_nfts_eth, offer_bayc, weth, borrower, lender, bayc, now, lender_key, borrower_broker, borrower_broker_fee, protocol_fee):
    ...
    loan_id = p2p_nfts_eth.create_loan(signed_offer, token_id, borrower, borrower_broker_fee, borrower_broker, sender=borrower)
...
def test_replace_loan_reverts_if_loan_invalid(p2p_nfts_eth, ongoing_loan_bayc, offer_bayc2):
    ...
    for loan in get_loan_mutations(ongoing_loan_bayc):
        ...
        with boa.reverts("invalid loan"):
            p2p_nfts_eth.replace_loan(loan, offer_bayc2, 0, ZERO_ADDRESS, sender=ongoing_loan_bayc.borrower)
...
def test_replace_loan_reverts_if_loan_defaulted(p2p_nfts_eth, ongoing_loan_bayc, now, offer_bayc2):
    ...
    with boa.reverts("loan defaulted"):
        p2p_nfts_eth.replace_loan(ongoing_loan_bayc, offer_bayc2, 0, ZERO_ADDRESS, sender=ongoing_loan_bayc.borrower)
...
def test_replace_loan_reverts_if_offer_not_signed_by_lender(p2p_nfts_eth, borrower_key, ongoing_loan_bayc, offer_bayc2):
    ...
    with boa.reverts("offer not signed by lender"):
        p2p_nfts_eth.replace_loan(ongoing_loan_bayc, signed_offer, 0, ZERO_ADDRESS, sender=ongoing_loan_bayc.borrower)
...
def test_replace_loan_reverts_if_funds_not_sent(p2p_nfts_eth, ongoing_loan_bayc, weth, offer_bayc2):
    ...
    with boa.reverts("invalid sent value"):
        p2p_nfts_eth.replace_loan(ongoing_loan_bayc, offer_bayc2, 0, ZERO_ADDRESS, sender=borrower, value=amount_to_settle - 1)
...
def test_replace_loan_reverts_if_collateral_locked(p2p_nfts_eth, p2p_control, ongoing_loan_bayc, now, offer_bayc2, weth):
    ...
    with boa.reverts("collateral locked"):
        p2p_nfts_eth.replace_loan(ongoing_loan_bayc, offer_bayc2, 0, ZERO_ADDRESS, sender=borrower, value=amount_to_settle)
...
def test_replace_loan(p2p_nfts_eth, ongoing_loan_bayc, offer_bayc2, now, bayc, weth):
    ...
    loan_id = p2p_nfts_eth.replace_loan(ongoing_loan_bayc, offer_bayc2, 0, ZERO_ADDRESS, sender=borrower, value=amount_to_settle)
...
def test_replace_loan_logs_event(p2p_nfts_eth, ongoing_loan_bayc, offer_bayc2, now, bayc, weth):
    ...
    principal = offer.principal
    amount_to_settle = ongoing_loan_bayc.amount + ongoing_loan_bayc.interest
    protocol_fee_amount = ongoing_loan_bayc.get_protocol_fee().interest_bps * ongoing_loan_bayc.interest // 10000
    broker_fee_amount = ongoing_loan_bayc.get_lender_broker_fee().interest_bps * ongoing_loan_bayc.interest // 10000
    borrower_broker_fee_amount = ongoing_loan_bayc.get_borrower_broker_fee().interest_bps * ongoing_loan_bayc.interest // 10000
    weth.deposit(value=principal, sender=lender)
    weth.approve(p2p_nfts_eth.address, principal, sender=lender)
...
def test_replace_loan_pays_lender(p2p_nfts_eth, ongoing_loan_bayc, offer_bayc2, weth):
    ...
    principal = offer.principal
    interest = loan.interest
    protocol_fee_amount = interest * ongoing_loan_bayc.get_protocol_fee().interest_bps // 10000
    broker_fee_amount = interest * ongoing_loan_bayc.get_lender_broker_fee().interest_bps // 10000
    lender_fee_amount = interest * ongoing_loan_bayc.get_borrower_broker_fee().interest_bps // 10000
    amount_to_settle = ongoing_loan_bayc.amount + ongoing_loan_bayc.interest
...
def test_replace_loan_pays_borrower_if_amount_to_settle_negative(p2p_nfts_eth, ongoing_loan_bayc, offer_bayc, weth, lender, lender_key):
    ...
    p2p_nfts_eth.replace_loan(ongoing_loan_bayc, sign_offer(offer, lender_key, p2p_nfts_eth.address), 0, ZERO_ADDRESS, sender=borrower)
...
def test_replace_loan_prorata_logs_event(p2p_nfts_eth, ongoing_loan_prorata, weth, now, offer_bayc2):
    ...
    boa.env.time_travel(seconds=actual_duration)
    loan_id = p2p_nfts_eth.replace_loan(loan, offer_bayc2, 0, ZERO_ADDRESS, sender=borrower, value=amount_to_settle)
    ...
    assert event.fees == [
        Fee(FeeType.PROTOCOL, 0, p2p_nfts_eth.protocol_fee(), p2p_nfts_eth.protocol_wallet()),
        Fee(FeeType.ORIGINATION, offer.origination_fee_amount, 0, new_lender),
        Fee(FeeType.LENDER_BROKER, 0, offer.broker_fee_bps, offer.broker_address),
        Fee(FeeType.BORROWER_BROKER, 0, 0, ZERO_ADDRESS),
    ]
...
def test_replace_loan_prorata_pays_lender(p2p_nfts_eth, ongoing_loan_prorata, weth, offer_bayc2):
    ...
    print(f"{amount_to_settle=}")
    boa.env.time_travel(seconds=actual_duration)
    p2p_nfts_eth.replace_loan(loan, offer_bayc2, 0, ZERO_ADDRESS, sender=borrower, value=amount_to_settle)
...
def test_replace_loan_settles_amounts(  # noqa: PLR0914
    ...
    protocol_fee_amount = interest * loan1.get_protocol_fee().interest_bps // 10000
    broker_fee_amount = interest * loan1.get_lender_broker_fee().interest_bps // 10000
    borrower_broker_fee_amount = interest * loan1.get_borrower_broker_fee().interest_bps // 10000
    ...
    boa.env.time_travel(seconds=actual_duration)
    loan2_id = p2p_nfts_eth.replace_loan(loan1, signed_offer2, 0, ZERO_ADDRESS, sender=borrower, value=max(0, -borrower_delta))
    ...
    loan2 = Loan(
        ...
        fees=[
            Fee(FeeType.PROTOCOL, 0, p2p_nfts_eth.protocol_fee(), p2p_nfts_eth.protocol_wallet()),
            Fee(FeeType.ORIGINATION, offer2.origination_fee_amount, 0, lender2),
            Fee(FeeType.LENDER_BROKER, 0, offer2.broker_fee_bps, offer2.broker_address),
            Fee(FeeType.BORROWER_BROKER, 0, 0, ZERO_ADDRESS),
        ],
