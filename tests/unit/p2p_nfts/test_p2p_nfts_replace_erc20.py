...
def test_replace_loan_transfers_principal_to_borrower(p2p_nfts_usdc, ongoing_loan_bayc, offer_bayc2, usdc):
    ...
    assert usdc.balanceOf(borrower) == initial_borrower_balance - amount_to_settle + principal - upfront_fees
...
def test_replace_loan_transfers_origination_fee_to_lender(p2p_nfts_usdc, ongoing_loan_bayc, offer_bayc2, usdc):
    ...
    usdc.approve(p2p_nfts_usdc.address, principal - origination_fee, sender=new_lender)

    usdc.approve(p2p_nfts_usdc.address, amount_to_settle, sender=borrower)
    p2p_nfts_usdc.replace_loan(ongoing_loan_bayc, offer_bayc2, 0, 0, ZERO_ADDRESS, sender=borrower)

    assert usdc.balanceOf(new_lender) == initial_lender_balance - principal + origination_fee
...
def test_replace_loan_pays_broker_fees(p2p_nfts_usdc, ongoing_loan_bayc, offer_bayc2, usdc):
    ...
    usdc.approve(p2p_nfts_usdc.address, new_principal, sender=new_lender)
...
def test_replace_loan_settles_amounts(  # noqa: PLR0914
    ...
    bayc.mint(borrower, token_id)
    bayc.approve(p2p_nfts_usdc.address, token_id, sender=borrower)
    usdc.approve(p2p_nfts_usdc.address, principal_loan1 - origination_fee, sender=lender)
